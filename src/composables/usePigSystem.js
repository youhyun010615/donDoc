...
export function usePigSystem() {
  ...
  function getCharacterGuideMessage(monthlyExpense, dailyBudget, currentMonth) {
    ...
    const fmt = (n) => new Intl.NumberFormat('ko-KR').format(Math.max(n, 0));

    const stageGuides = {
      5: `이번 달 남은 예산 ${fmt(remainBudget)}원\n앞으로 ${remainDays}일, 하루 ${fmt(recommendPerDay)}원 이하면 돼지가 집사에게 최고급 대우를 받을 수 있어요.`,
      4: `이번 달 남은 예산 ${fmt(remainBudget)}원\n앞으로 ${remainDays}일, 하루 ${fmt(recommendPerDay)}원 이하로 써봐요!\n농부가 맛있는 식량을 주러 올거에요`,
      3: `이번 달 남은 예산 ${fmt(remainBudget)}원\n하루 ${fmt(recommendPerDay)}원 이하로 줄이면 집이 업그레이드돼요.`,
      2: `지출을 줄여야 해요!\n남은 ${remainDays}일 동안 하루 ${fmt(recommendPerDay)}원 이하로 써야 도둑이 돼지 식량을 안 뺏어가요.`,
      1: `당장 지출을 멈추세요!\n도축사가 돼지를 죽이러 오기 전에 ${remainDays}일간 하루 ${fmt(recommendPerDay)}원으로 버텨야 해요.`,
    };

    return `${character.name} (페이스 ${character.pace}x)\n\n${stageGuides[character.stage]}`;
  }
